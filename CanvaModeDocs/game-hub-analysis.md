...
The hub currently integrates games like "Trivia Blitz", "Canva", "Ping Pong", and "Chronicles of the Abyss", and supports additional games in various states (alpha, beta, stable, deprecated). It features game discovery, categorization, feature flagging, visibility controls, and seamless navigation between different game experiences.
...
{
  label: "Trivia Blitz", // From game config
  to: "/games/trivia-blitz",
  category: "trivia",
  priority: 90
}
```
  ...
  "entries": [
    {
      ...
      "description": { "default": "Fast-paced quiz game where speed and accuracy win..." },
      "status": "stable",
...
- Includes at least one playable game (Trivia Blitz, Canva, etc.)
...
2. **Router Resolution**
   ...
   /games/trivia-blitz
     ↓
   Router matches `/games/trivia-blitz`
     ↓
...
3. **Game Page Renders**
   ...
   - Example: Trivia Blitz has `/games/trivia-blitz`, `/games/trivia-blitz/room/:roomId`
...
### Trivia Blitz Integration Example

Trivia Blitz integrates with the hub through:
...
**2. Hub Entry File** (`src/games/trivia-blitz/hubEntry.tsx`):
- Exports `getTriviaBlitzPreviewEntry()`: Provides entry metadata
...
- ✅ Routes work: `/games/trivia-blitz`, `/games/trivia-blitz/room/:roomId`
...
**Content:**
- Hardcoded minimal game entries
- Includes Trivia Blitz, Canva, Ping Pong, Chronicles of the Abyss (fully playable)
...
**Example Routes:**
- `/` → All Games grid
- `/games/trivia-blitz` → Trivia Blitz lobby
- `/games/trivia-blitz/room/:roomId` → Trivia Blitz game room
- `/games/canva` → Canva lobby
- `/games/canva/room/:roomId` → Canva collaborative room
...
│ │  <Outlet /> ← Game pages render here                    │ │
│ │                                                         │ │
│ │  Example: AllGames grid or Trivia Blitz lobby           │ │
│ │                                                         │ │
│ └─────────────────────────────────────────────────────────┘ │
└─────────────────────────────────────────────────────────────┘
```
...
│ │ [New]       │ │ [New]       │ │ [New]       │           │
│ │ Trivia      │ │ Canva       │ │ Ping Pong   │           │
│ │ Blitz       │ │             │ │             │           │
│ │ Fast-paced  │ │ Collaborative│ │ Classic     │           │
│ │ quiz...     │ │ drawing...  │ │ table tennis│           │
│ │ 2-12        │ │ 1-10        │ │ 1-2 players │           │
│ │ players     │ │ players     │ │             │           │
│ │             │ │             │ │             │           │
...
├── Sidebar Navigation
│   ├── All Games (link to /)
│   ├── Trivia Blitz (link to /games/trivia-blitz)
│   ├── Canva (link to /games/canva)
│   ├── Ping Pong (link to /games/ping-pong)
│   ├── Chronicles of the Abyss (link to /games/chronicles-of-the-abyss)
│   └── Other Games...
├── Header
│   └── Game Hub branding + mobile nav
└── Main Content (<Outlet />)
    ├── AllGames (at /)
    ├── Trivia Blitz Pages (at /games/trivia-blitz/*)
    ├── Canva Pages (at /games/canva/*)
    └── Other Game Pages
```
...
**Note:** Games provide their own context providers as needed. For example:
- Trivia Blitz uses `TriviaBlitzApp` wrapper with its own context
- Canva uses `CanvaApp` wrapper with its own context
...
└── /games (HubLayout still active)
    ├── /trivia-blitz (Trivia Blitz routes)
    │   ├── / (Lobby)
...
│   │   └── ...                    # Other game files
│   ├── canva/
│   │   ├── hubEntry.tsx           # Hub integration
│   │   ├── pages/
│   │   │   ├── Lobby.tsx          # Lobby
│   │   │   └── Room.tsx           # Collaborative room
│   │   └── ...                    # Other game files
│   └── ...                        # Other games
└── router/
...
- `id: string`: Unique identifier (e.g., "trivia-blitz")
...
- `category: string[]`: Game categories (e.g., ["trivia", "party"])
...
- Multiple routes (lobby, room)
- Full multiplayer support (1-10 players)
- Integrated with hub navigation
- Backend room management with separate `CanvaRoom` class

### Ping Pong

**Status:** Stable  
**Route:** `/games/ping-pong`  
**Integration:** Full (routes)

**Features:**
- Classic table tennis gameplay
...
**Backend Files:**
- `canvaRoom.js` - Game state management
- `canvaRoomRepository.js` - Room collection management
- Socket handlers for collaborative drawing events

**Frontend Files:**
- `CanvaContext.tsx` - React context for state management
- `Canvas.tsx` - Collaborative drawing canvas component
- `Lobby.tsx` - Room creation/joining
- `Room.tsx` - Main collaborative room
...
- Games provide their own context providers as needed (e.g., TriviaBlitzApp, CanvaApp wrappers)
