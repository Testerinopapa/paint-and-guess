...
import { SKIN_TONE_PRESETS, SKIN_TONE_COLORS } from "@/lib/avatarAssets";
...
export function SkinToneSelector({ selectedTone, onSelect }: SkinToneSelectorProps) {
  ...
  const handlePresetSelect = (presetId: string) => {
    console.debug('[SkinToneSelector] Preset selected', { 
      presetId, 
      color: SKIN_TONE_COLORS[presetId],
      previousTone: selectedTone 
    });
    onSelect(presetId);
  ...
  const handleCustomColorChange = (color: string) => {
    console.debug('[SkinToneSelector] Custom color changed', { 
      color, 
