...
if (redisUrl) {
  redisClients = await configureRedisAdapter(io, redisUrl);
}
...
async function configureRedisAdapter(ioInstance, url) {
  try {
    const pubClient = createRedisClient({ url });
    const subClient = pubClient.duplicate();
    await Promise.all([pubClient.connect(), subClient.connect()]);
    ioInstance.adapter(createAdapter(pubClient, subClient));
    logger.info({ url }, "Enabled Redis adapter for Socket.IO");
    return { pubClient, subClient };
  } catch (error) {
    logger.error({ err: error }, "Failed to initialize Redis adapter; falling back to in-memory adapter");
    return null;
  }
}
...
const shutdown = async () => {
  if (redisClients) {
    try {
      ...
      logger.info("Closed Redis adapter clients");
    } catch (error) {
      logger.error({ err: error }, "Failed to close Redis adapter clients cleanly");
    }
  }
};
